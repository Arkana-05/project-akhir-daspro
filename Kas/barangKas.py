...
def show_detail(db):
    ...
    cursor.execute("SELECT kode, SUM(qty) AS total_qty FROM kas_barang GROUP BY kode")
...
show_detail(db) #menampilkan fungsi show_data
...
def insert_data(db):
    ...
    while True:
        ...
        for i in range(jumlah_data):
            ...
            while True:  # Perulangan untuk validasi kode_barang
                kode_barang = input("Kode [PITAK/Sweety Slice] : ")  # Menghapus spasi berlebih jika ada
                if kode_barang == "PITAK" or kode_barang == "Sweety Slice":
            ...
            sql = "INSERT INTO kas_barang (kode, nama, qty, harga, total) VALUES (%s, %s, %s, %s, %s)"
...
def show_data(db):
    cursor = db.cursor()
    # MENGAMBIL SEMUA DATA YANG ADA DALAM TABEL kas_BARANG
    cursor.execute("SELECT * FROM kas_barang")
...
def update_data(db):
    ...
    kodea_barang = input("Kode Barang : ")
    nama_barang = input("Nama Barang : ")
    while True:
        try:
            qty_barang = int(input("Quantity Barang : "))
    ...
    sql = "UPDATE kas_barang SET kode=%s, nama=%s, qty=%s, harga=%s, total=%s WHERE id=%s"
    # untuk value nama variable nya harus sesuai dengan variable input di atas 
    value = (kodea_barang, nama_barang, qty_barang, harga_barang, total, _id)
...
def delete_data(db):
    ...
    # nama, qty hahrus sesuai dengan yang ada di tabel
    sql = "DELETE FROM kas_barang WHERE id=%s"
    # untuk value nama variable nya harus sesuai dengan variable input di atas 
    value = (_id,)
    cursor.execute(sql, value)
...
def search_data(db):
    ...
    sql = "SELECT kode, nama, qty, harga, total FROM kas_barang WHERE kode LIKE %s OR nama LIKE %s OR qty LIKE %s OR harga LIKE %s "
